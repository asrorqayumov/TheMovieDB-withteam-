import * as Tv from "../apis/tv-page.js";
...
export async function keywords_all(id){
  ...
  Tv.getTv(id).then(async (data) => {
    let keywords = await Tv.Tv_Keywords(id);
...
export async function coll_all (id){
  ...
  Tv.getTv(id).then(data => collection(data))
    ...
    setTimeout(() => {
      ...
      let favorite = data.favorite
        heart.addEventListener("click",()=>{
          ...
          Tv
          .category(id,type,favorite).then()
          console.log(data);
        })
    }, 2000);
  }
  Tv.states(id).then(data => state(data))
}
    ...
    setTimeout(() => {
      ...
      let watchlist = data.watchlist
        bookmark.addEventListener("click",()=>{
          ...
          Tv.watchlist(id,type,watchlist).then()
          console.log(data);
        })
    }, 2000);
  }
